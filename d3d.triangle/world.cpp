...
void CreateDeviceIndependentResources(ID2D1Factory **d2d1_factory,
                                      IDWriteFactory **dwrite_factory,
                                      IDWriteTextFormat **dwrite_text_format);

HRESULT CreateD3DDevice(D3D_DRIVER_TYPE const kD3DDriverType,
...
void DiscardDeviceDependentResources(ID3D11Device1 **d3d11_device,
                                     ID3D11DeviceContext1 **d3d11_device_context,
...
void CreateRenderTargetResources(HWND hwnd,
                                 ID3D11Device1 *d3d11_device,
                                 IDXGISwapChain1 *dxgi_swap_chain,
...
void DiscardRenderTargetResources(ID3D11DeviceContext1 **d3d11_device_context,
                                  ID3D11RenderTargetView **d3d11_frame_buffer_view,
                                  ID2D1RenderTarget **d2d1_back_buffer_render_target,
                                  ID2D1SolidColorBrush **d2d1_text_brush);

void CreateVertexShader(ID3D11Device *d3d11_device,
                        ID3DBlob **vsBlob,
                        ID3D11VertexShader **vertex_shader);

void CreatePixelShader(ID3D11Device *d3d11_device,
                       ID3DBlob **ps_blob,
                       ID3D11PixelShader **pixel_shader);

bool ShaderCompilerSucceeded(HRESULT hr, ID3DBlob *shader_compiler_error_blob);
...
HWND CreateWin32Window(HINSTANCE hinstance, LONG surface_width, LONG surface_height);
                                    ...
                                    IDXGISwapChain1 **dxgi_swap_chain)
{
    ...
    HRESULT hr = CreateD3DDevice(D3D_DRIVER_TYPE_HARDWARE, d3d11_device, d3d11_device_context);
    if (hr == DXGI_ERROR_UNSUPPORTED)
        hr = CreateD3DDevice(D3D_DRIVER_TYPE_WARP, d3d11_device, d3d11_device_context);
    assert(SUCCEEDED(hr));
...
void DiscardDeviceDependentResources(ID3D11Device1 **d3d11_device,
                                     ...
                                     ID2D1SolidColorBrush **d2d1_text_brush)
{
    SafeRelease(d2d1_text_brush);
    SafeRelease(d2d1_back_buffer_render_target);
    SafeRelease(d3d11_frame_buffer_view);
    SafeRelease(dxgi_swap_chain);
    SafeRelease(pixel_shader);
    SafeRelease(vertex_shader);
    SafeRelease(d3d11_device_context);
    SafeRelease(d3d11_device);
}
...
void Resize(HWND hwnd,
            ...
            ID2D1SolidColorBrush **d2d1_text_brush)
{
    DiscardRenderTargetResources(d3d11_device_context,
                                 d3d11_frame_buffer_view,
                                 d2d1_back_buffer_render_target,
                                 d2d1_text_brush);
    HRESULT hr = (*dxgi_swap_chain)->ResizeBuffers(0, 0, 0, DXGI_FORMAT_UNKNOWN, 0);
    assert(SUCCEEDED(hr));
    CreateRenderTargetResources(hwnd,
                                *d3d11_device,
                                *dxgi_swap_chain,
                                *d2d1_factory,
                                d3d11_frame_buffer_view,
                        ...
                        ID3D11VertexShader **vertex_shader)
{
    ...
    assert(ShaderCompilerSucceeded(hr, shader_compiler_error_blob));
...
HWND CreateWin32Window(HINSTANCE hinstance, LONG surface_width = 400, LONG surface_height = 300)
{
    WNDCLASSEXW wc{};
...
{
    HWND hwnd = CreateWin32Window(hinstance, 1024, 768);
    ...
    CreateDeviceIndependentResources(&d2d1_factory, &dwrite_factory, &dwrite_text_format);
    ...
    CreateDeviceDependentResources(hwnd, &d3d11_device, &d3d11_device_context, &dxgi_swap_chain);
    ...
    CreateVertexShader(d3d11_device, &vs_blob, &vertex_shader);
    ...
    CreatePixelShader(d3d11_device, &ps_blob, &pixel_shader);
    // ps_blob->Release();
    ...
    while (running)
    {
        ...
        if (g_device_lost)
        {
            DiscardDeviceDependentResources(&d3d11_device,
                                            &d3d11_device_context,
                                            &d3d11_frame_buffer_view,
                                            &vertex_shader,
                                            &pixel_shader,
